(function(ng) {	
    ...
    filtrosRecibosComponent.controller = function filtrosRecibosComponentComponentController($location, $routeParams, validacionesService, sharePropertiesService, BusquedaService, TiposService, ColectivoService, uiGridConstants, BASE_SRC, $mdDialog, constantsTipos){
    	var vm=this;
    	var json = {};
    	vm.vista = 1;
		vm.tipos = {};
    	...
    	this.$onInit = function(){
    		...
    		if(vm.parentApp.listServices.listSituaPoliza != null && vm.parentApp.listServices.listSituaPoliza.length > 0){
    			vm.tipos.situaPoliza = vm.parentApp.listServices.listSituaPoliza;
    		} else {
    			TiposService.getSituaPolizas({})
    			.then(function successCallback(response){
    				if(response.status == 200){
    					vm.tipos.situaPoliza = response.data.TIPOS.TIPO;
    					vm.parentApp.listServices.listSituaPoliza = vm.tipos.situaPoliza;
    				}
    			}, function callBack(response){
    				if(response.status == 406 || response.status == 401){
                    	vm.parent.logout();
    	...
    	this.loadTemplate=function(){
    		return BASE_SRC+"filtros/filtros.view/filtros.recibo.html";
    	}
    	
    	vm.buscar = function(tipo){
			
			if (vm.form.IS_NEW != null) {
				delete vm.form.IS_NEW;
			}
			
			setTimeout(function() {
				angular.forEach(vm.form, function(value, key) {
					if((value == null || (value.value == "" || value.value==null)) && key != 'OPOLIZA'){
						delete vm.form[key];							
					}
				});
...
//                    vm.form.LST_COLECTIVOS.value.push(colectivo.ID_TIPO_POLIZA);
    ...
    ng.module('App').component('filtrosRecibosApp', Object.create(filtrosRecibosComponent));    
})(window.angular);