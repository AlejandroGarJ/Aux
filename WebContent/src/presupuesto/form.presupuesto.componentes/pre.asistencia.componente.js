(function (ng) {
	...
	var preAsistenciaComponent = {
		controllerAs: '$ctrl',
		template: '<div ng-include="$ctrl.loadTemplate()"></div>',
		$inject: ['$rootScope', '$location', '$timeout', '$filter', '$mdDialog', 'PresupuestoService', 'TiposService', 'BusquedaService', 'SolicitudService', 'LocalidadesService', 'ValidacionPagoService', 'HogarService'],
		require: {
			parent: '^sdApp',
			pre: '^sdPresupuesto',
	...
	preAsistenciaComponent.controller = function preAsistenciaComponentControler($rootScope, $location, $timeout, $filter, $mdDialog, PresupuestoService, TiposService, BusquedaService, SolicitudService, LocalidadesService, ValidacionPagoService, HogarService) {
		var vm = this;
		vm.tipos = {};
		vm.calendar = {};
		vm.form = {};
		...
		this.$onInit = function () {
			...
			TiposService.getCompania({})
				.then(function successCallback(response) {
					...
					//console.log('Compañias: ', vm.tipos.compania);
				});
			...
			BusquedaService.buscar({
				...
				.then(function successCallback(response) {
					if (response.status == 200) {
						...
						// console.log(response.data.POLIZAS.POLIZA);
			...
			if(vm.pre.detalles.ID_PRESUPUESTO==undefined){
				...
				objJSON = {
						"PRESUPUESTO_ASISTENCIA_VIAJE": {
							"DATOS_PAGADOR": DATOS_PAGADOR,
							"DATOS_PAGO": { "ID_FORMAPAGO": 2 }
						}
				}
//				objJSON['PRESUPUESTO_ASISTENCIA_VIAJE'] = {DATOS_PAGADOR};
			//	vm.pre.detalles.XM_ENVIADO = x2js.js2xml(objJSON);
			//	vm.datos = x2js.xml2js(vm.pre.detalles.XM_ENVIADO).PRESUPUESTO_ASISTENCIA_VIAJE;
    		...
    		if (vm.pre.detalles.ID_PRESUPUESTO != undefined && vm.pre.detalles.ID_PRESUPUESTO != null) {
				...
				console.log('Datos', vm.datos);
				//console.log('Tarifas', vm.tarifas);
				
				if(vm.pre.detalles.XM_ENVIADO != null && vm.pre.detalles.XM_ENVIADO != undefined){
		...
		this.loadTemplate = function () {
			...
			if (vm.llave=="presupuesto" || vm.llave=="nuevo") {
				return "src/presupuesto/form.presupuesto.view/pre.asistencia.html";
			} else if (vm.llave == "contratar") {
				return "src/presupuesto/form.presupuesto.view/pre.contratar.asistencia.html";
			} else if (vm.llave == "resumen") {
				return "src/presupuesto/form.presupuesto.view/resumen.view/pre.resumen.asistencia.html";
			}
		}
		...
		//TOMADOR ES PAGADOR
		vm.tomadorEsPagador = function(value){
			if(value == '01'){
				vm.datos.DATOS_TOMADOR = {};
				vm.datos.DATOS_TOMADOR.IN_TOMADOR_ES_PAGADOR = '01';
			}else if(value == '02'){
				...
				vm.datos.DATOS_TOMADOR.IN_TOMADOR_ES_PAGADOR = '02';
			}
		}
		
		//Nuevo asegurado
		vm.addAsegurado = function () {
			if (vm.newAsegurado.length <= 10)
				vm.newAsegurado.push({});
			console.log('Numero asegurados', vm.newAsegurado.length);
			console.log('Asegurados: ', vm.newAsegurado);
		}
		...
		vm.validarTarifa = function () {
			...
			var objJSON = {};
    		objJSON['PRESUPUESTO_ASISTENCIA_VIAJE'] = vm.datos;
		...
		vm.contratar = function(){
			...
			for(persona in objJSON['PRESUPUESTO_ASISTENCIA_VIAJE']) {
    			for(dato in objJSON['PRESUPUESTO_ASISTENCIA_VIAJE'][persona]) {
    				if(objJSON['PRESUPUESTO_ASISTENCIA_VIAJE'][persona][dato] === '' || objJSON['PRESUPUESTO_ASISTENCIA_VIAJE'][persona][dato] === undefined || objJSON['PRESUPUESTO_ASISTENCIA_VIAJE'][persona][dato] === null || objJSON['PRESUPUESTO_ASISTENCIA_VIAJE'][persona][dato] === 'NaN'){
    					delete objJSON['PRESUPUESTO_ASISTENCIA_VIAJE'][persona][dato];
    				}
    			}
    		}
		...
		vm.validar = function(form2Validate) {
            if (form2Validate) {
            	...
            	if(vm.llave!="contratar"){
            		vm.pre.tarificarPresupuestos(vm.datos, 'PRESUPUESTO_ASISTENCIA_VIAJE');
            	}else if(vm.indice<5){
...
//                vm.pre.tarificarPresupuestos(vm.datos, 'PRESUPUESTO_VIDA');
	...
	ng.module('App').component('preAsistenciaApp', Object.create(preAsistenciaComponent));
