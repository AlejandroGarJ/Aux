(function(ng) {	
    ...
    var preDentalComponent = {
    		controllerAs: '$ctrl',
    		template:'<div ng-include="$ctrl.loadTemplate()"></div>',
    		$inject:['$rootScope', '$location', '$timeout', 'TiposService', 'BusquedaService', 'PresupuestoService', 'SolicitudService', '$mdDialog', 'LocalidadesService', 'ValidacionPagoService', 'HogarService'],
    		require: {
            	parent:'^sdApp',
            	pre:'^sdPresupuesto',
    ...
    preDentalComponent.controller = function preDentalComponentControler($rootScope, $location, $timeout, TiposService, BusquedaService, PresupuestoService, SolicitudService, $mdDialog, LocalidadesService, ValidacionPagoService, HogarService) {
    	var vm=this;
    	vm.tipos = {};
    	vm.calendar = {};
    	vm.form = {};
...
//				objJSON['PRESUPUESTO_DENTAL'] = {DATOS_PAGADOR};
//				vm.pre.detalles.XM_ENVIADO = x2js.js2xml(objJSON);
//				vm.datos = x2js.xml2js(vm.pre.detalles.XM_ENVIADO).PRESUPUESTO_DENTAL;
...
//	    		vm.datos = x2js.xml2js(aux.XM_ENVIADO).PRESUPUESTO_DENTAL;
	    		...
	    		vm.datosPagador = vm.datos.DATOS_PAGADOR;
	    		
	    		if(vm.datos.DATOS_PAGO.FD_INICIO != undefined && typeof vm.datos.DATOS_PAGO.FD_INICIO === 'string'){
	    			vm.datos.DATOS_PAGO.FD_INICIO = new Date(vm.datos.DATOS_PAGO.FD_INICIO.substr(0, 10)); 
	    		}
    	...
    	this.loadTemplate=function(){
			...
			if (vm.llave == "presupuesto" || vm.llave=="nuevo") {
				return "src/presupuesto/form.presupuesto.view/pre.dental.html";
			} else if (vm.llave == "contratar") {
				return "src/presupuesto/form.presupuesto.view/pre.contratar.dental.html";
			} else if (vm.llave == "resumen") {
				return "src/presupuesto/form.presupuesto.view/resumen.view/pre.resumen.dental.html";
			}
    	}
	    ...
	    vm.aseguradoEsTomador = function(valor){
	    	if(valor=='01'){
	    		...
	    		vm.datos.DATOS_PERSONA_ASEGURADA.IN_TOMADOR_ES_ASEGURADO='01';
	    	}else if(valor=='00'){
	    		vm.datos.DATOS_PERSONA_ASEGURADA = {};
	    		vm.datos.DATOS_PERSONA_ASEGURADA.IN_TOMADOR_ES_ASEGURADO='00';
	    	}
	    }
	    ...
	    vm.tomadorEsPagador = function(valor){
	    	if(valor=='01'){
	    		if(vm.datos.DATOS_PAGADOR != undefined){
					...
					if(vm.datos.DATOS_PAGADOR.FD_NACIMIENTO != undefined && typeof vm.datos.DATOS_TOMADOR.FD_NACIMIENTO === 'string') {
    			...
    			vm.datos.DATOS_TOMADOR = {};
    			vm.datos.DATOS_TOMADOR.IN_TOMADOR_ES_PAGADOR = '00';
	    	}
	    }
	  ...
	  //Identificar la localidad por código postal
		...
		vm.validarTarifa = function () {
			...
			var objJSON = {};
    		objJSON['PRESUPUESTO_DENTAL'] = vm.datos;
...
//    		datosLlamada.OPRESUPUESTO.XM_ENVIADO = x2js.js2xml(objJSON);
		...
		vm.contratar = function(){
			...
			for(persona in objJSON['PRESUPUESTO_DENTAL']) {
    			for(dato in objJSON['PRESUPUESTO_DENTAL'][persona]) {
    				if(objJSON['PRESUPUESTO_DENTAL'][persona][dato] === '' || objJSON['PRESUPUESTO_DENTAL'][persona][dato] === undefined || objJSON['PRESUPUESTO_DENTAL'][persona][dato] === null || objJSON['PRESUPUESTO_DENTAL'][persona][dato] === 'NaN'){
    					delete objJSON['PRESUPUESTO_DENTAL'][persona][dato];
    				}
    			}
    		}
...
//			vm.presupuesto.OPRESUPUESTO.XM_ENVIADO = x2js.js2xml(objJSON);
		...
		vm.validar = function(form2Validate) {
            if (form2Validate) {
            	...
            	if(vm.llave!="contratar"){
            		vm.pre.tarificarPresupuestos(vm.datos, 'PRESUPUESTO_DENTAL');
            	}else if(vm.indice<5){
    ...
    ng.module('App').component('preDentalApp', Object.create(preDentalComponent));
