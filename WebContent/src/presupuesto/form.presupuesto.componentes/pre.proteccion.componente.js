(function(ng) {	
    ...
    preProteccionComponent.controller = function preProteccionComponentControler($rootScope, $location, $timeout, $mdDialog, TiposService, BusquedaService, SolicitudService, PresupuestoService, LocalidadesService, ValidacionPagoService, HogarService) {
    	var vm=this;
    	vm.tipos = {};
    	vm.calendar = {};
    	vm.form = {};
    	...
    	this.$onInit = function(){
    		...
    		if(vm.pre.detalles.ID_PRESUPUESTO==undefined){
				...
				objJSON = {
						'PRESUPUESTO_JURIDICA': {
							'DATOS_PAGADOR': DATOS_PAGADOR,
							"DATOS_PAGO": { "ID_FORMAPAGO": 2 }
						}
				}
				
//				objJSON['PRESUPUESTO_JURIDICA'] = {DATOS_PAGADOR};
//				vm.pre.detalles.XM_ENVIADO = x2js.js2xml(objJSON);
//				vm.datos = x2js.xml2js(vm.pre.detalles.XM_ENVIADO).PRESUPUESTO_JURIDICA;
...
//	    		vm.datos = x2js.xml2js(aux.XM_ENVIADO).PRESUPUESTO_JURIDICA;
    	...
    	this.loadTemplate=function(){
			...
			if (vm.llave == "presupuesto" || vm.llave=="nuevo" ) {
				return "src/presupuesto/form.presupuesto.view/pre.proteccion.html";
			} else if (vm.llave == "contratar") {
				return "src/presupuesto/form.presupuesto.view/pre.contratar.proteccion.html";
			} else if (vm.llave=="resumen") {
				return "src/presupuesto/form.presupuesto.view/resumen.view/pre.resumen.proteccion.html";
			}
    	}
	    ...
	    vm.tomadorEsPagador = function(valor){
	    	...
	    	for(atributo in vm.datos.DATOS_TOMADOR){
	    		vm.datos.DATOS_PERSONA_ASEGURADA[atributo] = vm.datos.DATOS_TOMADOR[atributo];
			}
			vm.datos.DATOS_PERSONA_ASEGURADA.IN_TOMADOR_ES_ASEGURADO = 1;
	    }
	    ...
	    vm.aseguradoEsTomador = function(valor){
	    	if(valor=='1'){
	    		...
	    		if(vm.datosAsegurado!=undefined){
	    			vm.datos.DATOS_PERSONA_ASEGURADA = vm.datosAsegurado;
	    		}
	    	}
	    }
	  ...
	  //Identificar la localidad por código postal
		...
		vm.validarTarifa = function () {
			...
			var objJSON = {};
    		objJSON['PRESUPUESTO_JURIDICA'] = vm.datos;
...
//    		vm.datosLlamada.OPRESUPUESTO.XM_ENVIADO = x2js.js2xml(objJSON);
		...
		vm.contratar = function(){
			...
			for(persona in objJSON['PRESUPUESTO_JURIDICA']) {
    			for(dato in objJSON['PRESUPUESTO_JURIDICA'][persona]) {
    				if(objJSON['PRESUPUESTO_JURIDICA'][persona][dato] === '' || objJSON['PRESUPUESTO_JURIDICA'][persona][dato] === undefined || objJSON['PRESUPUESTO_JURIDICA'][persona][dato] === null || objJSON['PRESUPUESTO_JURIDICA'][persona][dato] === 'NaN'){
    					delete objJSON['PRESUPUESTO_JURIDICA'][persona][dato];
    				}
    			}
    		}
		...
		vm.validar = function(form2Validate) {
            if (form2Validate) {
            	...
            	if(vm.llave!="contratar"){
            		vm.pre.tarificarPresupuestos(vm.datos, 'PRESUPUESTO_JURIDICA');
            	}else if(vm.indice<5){
...
//                vm.pre.tarificarPresupuestos(vm.datos, 'PRESUPUESTO_VIDA');
    ...
    ng.module('App').component('preProteccionApp', Object.create(preProteccionComponent));
