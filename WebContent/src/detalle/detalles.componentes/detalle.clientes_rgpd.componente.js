(function(ng) {	
    ...
    clienteRgpdComponent.controller = function clienteRgpdComponentControler($q, $scope, sharePropertiesService, $location, $mdDialog, $uibModal, $translate, validacionesService, BusquedaService, ClienteService, TiposService, uiGridConstants, BASE_SRC, LocalidadesService, $window, HogarService, FicherosService, CommonUtils, constantsTipos) {
    	var vm=this;
    	...
    	this.$onInit = function(bindings) {
    		...
    		if(vm.busquedaCliente != null && vm.busquedaCliente != undefined && vm.busquedaCliente.permisos != undefined){
    			...
    			if(vm.parent.pre.getPermissions != undefined){
            		vm.permisos = vm.parent.pre.getPermissions('rgpd_clientes');
        		}
    		}
...
//                  Cargar dashboard en resumen cliente
    	...
    	this.loadTemplate = function() {
			if(vm.permisos != undefined && vm.permisos.EXISTE != false) {
                return BASE_SRC + "detalle/detalles.views/detalle.clientes_rgpd.html";
            } else {
		...
		vm.updateDir = function (valor,index) {
			if (valor.length == 5 && vm.datos.CO_NACIONALIDAD != "DEU") {
				vm.localidades = [];
...
//			}
    ...
    ng.module('App').component('clienteRgpd', Object.create(clienteRgpdComponent));
