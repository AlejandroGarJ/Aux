(function(ng) {	
    ...
    polizaRgpdComponent.controller = function polizaRgpdComponentControler($timeout, $scope, $q, $window, sharePropertiesService, $location, $timeout, $mdDialog, $translate, BusquedaService, TiposService, PolizaService, validacionesService, BASE_SRC, ClienteService, DescargaService, ValidacionPagoService, HijosService, IdentidadService, EmpresaService, ConversorService, FicherosService, MovilService, ExportService, SolicitudService, HogarService, constantsTipos){
    	var vm=this;
    	...
    	this.$onInit = function() {
    		...
    		if(vm.busquedaPoliza != undefined && vm.busquedaPoliza.permisos != undefined){
        		...
        		if(vm.parent.pre.getPermissions != undefined){
            		vm.permisos = vm.parent.pre.getPermissions('rgpd_polizas');
        		}
    		}
    		...
    		if(vm.parent.parent.url == 'polizas' || vm.parent.parent.url == 'rgpd_polizas' || vm.parent.parent.url == 'recibos' || vm.parent.parent.url == 'ultRecibos' || vm.parent.parent.url == 'renting' || vm.parent.parent.url == 'siniestros' || vm.parent.parent.url == 'presupuestos')
    			vm.medida = 276;
    		else if(vm.parent.parent.url == 'clientes' || vm.parent.parent.url == 'rgpd_clientes')
    			vm.medida = 315;
    		...
    		if(vm.dato != null){
    			...
    			$q.all({'datosRequest':validacionesService.getDataPoliza(), 'dictRequest':validacionesService.getDictPoliza()}).then(function(data){
    		...
    		SolicitudService.getTiposSolicitudFilter({ "ID_RAMO": vm.dato.ID_RAMO, "ID_COMPANIA": vm.dato.ID_COMPANIA, "ID_COMP_RAMO_PROD": vm.dato.ID_COMP_RAMO_PROD })
			.then(function successCallback(response){
    	...
    	this.loadTemplate = function() {
			if(vm.permisos != undefined && vm.permisos.EXISTE != false) {
                return BASE_SRC + "detalle/detalles.views/detalle.polizas_rgpd.html";
            } else {
			...
			.then(function successCallback(response) {
				if (response.data.ID_RESULT == 0) {
                    ...
                    if(vm.dato.JS_ENVIADO != null && vm.dato.JS_ENVIADO != undefined && vm.dato.JS_ENVIADO != ""){
						...
						if (vm.dato.ID_COMP_RAMO_PROD == 5 || vm.dato.ID_COMP_RAMO_PROD == 6) {
					...
					//Para ciberriesgo
					if (vm.dato.ID_RAMO == 103 && (vm.dato.ID_COMP_RAMO_PROD == 5 || vm.dato.ID_COMP_RAMO_PROD == 6 || vm.dato.ID_COMP_RAMO_PROD == 25 || vm.dato.ID_COMP_RAMO_PROD == 29) && vm.bloques != null && vm.bloques.CIBERRIESGO != null) {
					...
					.then(function successCallback(response) {
						...
						if (vm.dato.ID_RAMO == 103 && (vm.dato.ID_COMP_RAMO_PROD == 5 || vm.dato.ID_COMP_RAMO_PROD == 6 || vm.dato.ID_COMP_RAMO_PROD == 25 || vm.dato.ID_COMP_RAMO_PROD == 29) && vm.bloques != undefined && vm.bloques.CIBERRIESGO != null) {
					...
					if(vm.appParent.listServices.listCantidadAsegurada_cp != null && vm.appParent.listServices.listCantidadAsegurada_cp.length > 0) {
						...
						if(vm.bloques && vm.bloques.CIBERPARTICULARES != null && vm.bloques.CIBERPARTICULARES.AMMOUNT_OPTION != null) {
							vm.bloques.CIBERPARTICULARES.AMMOUNT_OPTION = vm.amountCP.find(x => x.CO_TIPO == vm.bloques.CIBERPARTICULARES.AMMOUNT_OPTION);    
						}
					} else {
						TiposService.getTipos({
		...
		vm.guardarGarantia = function(){
    		...
    		PolizaService.addPoliza({"LST_GARANTIAS":rowsUpdate, "NU_POLIZA":vm.dato.NU_POLIZA, "ID_SITUAPOLIZA":vm.dato.ID_SITUAPOLIZA, "ID_POLIZA":vm.dato.ID_POLIZA, "IT_VERSION":vm.dato.IT_VERSION})
			.then(function successCallback(response){
    	...
    	vm.gridOptions = {
    		    ...
    		    columnDefs: [
    		      ...
    		      {field: 'NU_PORCENTAJE',
    		    	  ...
    		    	  cellTooltip: function(row){return row.entity.NU_PORCENTAJE}
    		      },
    		      {field: 'NU_COMISION',
    		    	  enableCellEdit: true,
    		    	  displayName: 'Comisión',
    	...
    	vm.cargarHistoricoPoliza = function () {
    		...
    		PolizaService.getHistoricoPoliza(json)
			.then(function successCallback(response){
				if(response.data.ID_RESULT == 0 && response.data.POLIZAS != null && response.data.POLIZAS.POLIZA.length > 0){
                 	vm.gridOptionsHistorico.data = response.data.POLIZAS.POLIZA;
				} else {
    	...
    	vm.gridOptionsHistorico = {
    		    ...
    		    columnDefs: [
					...
					{field: 'IM_PRIMA_TOTAL', displayName: 'Prima total', cellFilter: 'currency:"€" : 2'},
					{field: 'FD_INICIO', displayName: 'Fecha efecto',  cellFilter: 'date:\'dd/MM/yyyy\'',},
					{field: 'FD_VENCIMIENTO', displayName: 'Fecha vencimiento',  cellFilter: 'date:\'dd/MM/yyyy\'',},
	 ...
	 //recuperar documentación ya subida (desde padre)
      ...
      //meter desde lista padre los documentos recuperados
       ...
       vm.mostrarBtnNuevoSiniestro = function () {
    	   var listProductosAceptados = [3,4,5,6,25,28];
    	   if (vm.dato != null && vm.dato.ID_COMP_RAMO_PROD != null && listProductosAceptados.includes(vm.dato.ID_COMP_RAMO_PROD) && vm.dato.ID_SITUAPOLIZA == 1 && vm.readOnly != true) {
    		   return true;
    	   } else return false
       }
       ...
       vm.enviarPolizaPdf = function () {
    	   ...
    	   if (vm.dato.ID_COMP_RAMO_PROD != 10 && vm.dato.ID_COMP_RAMO_PROD != 11 && vm.dato.ID_COMP_RAMO_PROD != 12) {
    		    var valor = vm.dato.ID_POLIZA;
   			    if(vm.dato.tomador!=undefined && vm.dato!=undefined )
	    	...
	    	md.gridComisionistas = {
	   		    ...
	   		    columnDefs: [
	   		    	{field: 'NO_COMPANIA', displayName: 'Colaborador', enableCellEdit: false},
						...
						{field: 'NU_COMISION', displayName: 'Comisión', cellTemplate: '<div ng-if="row.entity.NU_COMISION!=undefined" class="ui-grid-cell-contents">{{ grid.appScope.$ctrl.formatAmount(row.entity.NU_COMISION)}} €</div>', enableCellEdit: false},
    ...
    ng.module('App').component('polizaRgpd', Object.create(polizaRgpdComponent));
