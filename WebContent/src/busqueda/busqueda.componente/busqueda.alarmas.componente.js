    ...
    busquedaComponent.controller = function busquedaComponentController($q, $location, $timeout, $window,validacionesService, sharePropertiesService, BusquedaService, BASE_SRC){
    	...
    	this.$onInit = function(){
			...
			if(/pendientes/.test(url)) {
				...
				vm.parent.buscar({'CO_USU_RECEPTOR': vm.parent.perfil}, "alarmas");
			}
    	}
    	...
    	vm.gridOptions = {
                ...
                gridFooterTemplate: '<div class="leyendaInferior">' +
                						'<div class="contenedorElemento"><a ng-if="grid.appScope.$ctrl.parent.url == \'alarmas\'" ng-click="grid.appScope.$ctrl.parent.recargarListado()"><md-icon>autorenew</md-icon></a></div>' +
            						'</div>',
...
//    		      {field: 'DS_TIPOEMPLEADO', displayName: 'S', cellTooltip: function(row){return row.entity.DS_TIPOEMPLEADO}},
    	...
    	this.loadTemplate=function(){
    		return BASE_SRC+"busqueda/busqueda.view/busqueda.alarmas.html";
    	}
    ...
    ng.module('App').component('busquedaAlarmas', Object.create(busquedaComponent));
